...
class LegalReviewPage extends StatefulWidget {
  const LegalReviewPage({super.key, this.loadBundle});

  /// Allows deterministic UI tests without replacing the real asset-loader tests.
  final Future<LegalReviewBundle> Function()? loadBundle;
...
class _LegalReviewPageState extends State<LegalReviewPage> {
  late final Future<LegalReviewBundle> _bundle =
      widget.loadBundle?.call() ?? LegalReviewBundle.load();
      ...
      body: FutureBuilder<LegalReviewBundle>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasError) {
            return const Center(
              child: Text('Legal documents unavailable. No terms accepted.'),
            );
          }
          ...
          return ListView(
            ...
            children: [
              const Text(
                'DRAFTS ONLY — NOT APPROVED OR ACCEPTED',
                style: TextStyle(fontWeight: FontWeight.bold),
              ),
              const Text(
                'These offline review copies do not replace live privacy notices, store terms or recording consent.',
              ),
