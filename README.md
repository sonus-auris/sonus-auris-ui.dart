...
- **Release blocker (DEN-205):** the hard 100-hour plaintext ceiling must delete expired audio and sensitive companion artifacts even when uploads are disabled, offline, pending, or failing. The current eligibility rule still protects unuploaded files indefinitely and must be replaced before store submission.
