...
exact commit SHA, builds with the pinned Flutter image in an init container,
and serves the immutable result from an unprivileged, read-only nginx container.
The cluster gateway exposes it at `/sonus-console/`.

Promotion is intentionally declarative:

1. Merge and push a green console commit.
...
- Add `console.sonusauris.app` in Cloudflare, attach it to the cluster gateway,
  and issue/rotate TLS through the cluster's declared certificate path.
