...
- **To an optional processing provider:** only when you explicitly enable a
  ...
  feature (for example the bounded audio excerpt needed by an external
  recognition provider for that request). Connected-storage encryption does not
  apply to an excerpt deliberately sent for recognition.
